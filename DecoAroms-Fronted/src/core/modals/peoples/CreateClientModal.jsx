...
const rutValidator = (rut) => {
    // La regex ahora espera la K en mayúscula, ya que la formateamos automáticamente
...
// Estado inicial del formulario reseteado
...
const CreateClientModal = ({ show, handleClose, onClientCreated }) => {
    ...
    // --- Hooks de disponibilidad en tiempo real (SIN CAMBIOS) ---
    // Hook para el RUT
    useEffect(() => {
        const currentRut = formData.rut;
        if (!rutValidator(currentRut)) {
    ...
    useEffect(() => {
        ...
        if (currentTelefono.length < MAX_PHONE_LENGTH) {
    ...
    const handleRutChange = (e) => {
        ...
        // 1. Limpiar: quitar todo lo que no sea número o k/K.
        let cleanValue = value.replace(/[^0-9kK]/g, "").toUpperCase();

        let formattedRut = cleanValue;
        // 2. Formatear: si hay más de 1 caracter, poner el guión antes del último.
        if (cleanValue.length > 1) {
            const body = cleanValue.slice(0, -1);
            const dv = cleanValue.slice(-1);
            formattedRut = `${body}-${dv}`;
        }
        // 3. Limitar longitud máxima (12.345.678-K -> 10 caracteres)
    ...
    const handleEmailChange = (e) => {
        ...
        setFormData((prev) => ({ ...prev, correo: sanitizedValue }));
        clearFieldErrors('correo');
    };

    const handlePhoneChange = (e) => {
        let value = e.target.value;
        // Asegurar que el prefijo siempre esté
        if (!value.startsWith(PHONE_PREFIX)) value = PHONE_PREFIX;
        // Limpiar todo lo que no sea dígito después del prefijo
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const newClient = await crearCliente(dataToSend);
            Swal.fire({
                title: "¡Creado!",
                text: `El cliente ${newClient.nombre}${newClient.apellido ? ` ${newClient.apellido}` : ""} ha sido creado.`,
                icon: "success",
                timer: 2000,
            ...
            if (errorData.details) {
                ...
                setApiError(errorData.error || errorData.message || "Error desconocido al crear el cliente.");
            }
        } finally {
            ...
            <Form noValidate onSubmit={handleSubmit}>
                <Modal.Body>
                    ...
                    <Form.Group className="mb-3">
                        ...
                        {!availabilityStatus.rut.checked && !validationErrors.rut && (
                            <Form.Text className="text-muted">Formato: 12345678-K</Form.Text>
                        )}
                    </Form.Group>
