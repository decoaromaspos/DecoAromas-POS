import React from 'react';
import { Document, Page, Text, View, StyleSheet, Image } from '@react-pdf/renderer';
import PropTypes from 'prop-types';

// Logo en Base64
const LOGO_BASE64 = 'data:image/jpeg;base64,';

const BASE_COL_PROPS = {
    borderStyle: 'solid',
    borderWidth: 1,
    borderColor: '#000',
    borderTopWidth: 0,
    borderLeftWidth: 0,
    padding: 4,
    fontSize: 7.5,
};

// Función de utilidad para formatear moneda
const formatCurrency = (amount) => {
    if (amount === null || amount === undefined) return '$0';
    return `$${(amount).toLocaleString('es-CL', { minimumFractionDigits: 0 })}`;
};

// Función para formatear la fecha
const formatearFecha = (fechaISO) => {
    if (!fechaISO) return '-';
    const fecha = new Date(fechaISO);
    return fecha.toLocaleDateString('es-CL') + ' ' + fecha.toLocaleTimeString('es-CL', {
        hour: '2-digit',
        minute: '2-digit'
    });
};

const styles = StyleSheet.create({
    page: {
        flexDirection: 'column',
        backgroundColor: '#ffffff',
        padding: 30,
        fontFamily: 'Helvetica',
    },
    header: {
        fontSize: 18,
        textAlign: 'center',
        marginBottom: 10,
        fontWeight: 'bold',
        color: '#000000ff',
    },
    subheader: {
        fontSize: 14,
        textAlign: 'center',
        marginBottom: 20,
        fontWeight: 'normal',
    },
    logo: {
        width: 60,
        height: 60,
        alignSelf: 'center',
        marginBottom: 10,
    },
    infoContainer: {
        flexDirection: 'row',
        justifyContent: 'flex-start',
        marginBottom: 20,
        borderBottomWidth: 1,
        borderBottomColor: '#aaa',
        paddingBottom: 10,
    },
    infoBlock: {
        fontSize: 9,
        lineHeight: 1.5,
    },
    // ESTILOS DE TABLA CON BORDES
    table: {
        display: 'table',
        width: 'auto',
        marginBottom: 10,
        borderStyle: 'solid',
        borderWidth: 1,
        borderColor: '#000',
    },
    tableRow: {
        margin: 'auto',
        flexDirection: 'row',
    },
    // Estilos de Encabezado
    tableColHeader: {
        ...BASE_COL_PROPS,
        backgroundColor: '#EAEAEA',
        fontWeight: 'bold',
        textAlign: 'center',
        borderBottomWidth: 1,
    },
    colWidth: { width: '5%' },
    colWidthCode: { width: '15%' }, // Cód. Barras
    colWidthProd: { width: '35%' }, // Producto
    colWidthQty: { width: '8%' },
    colWidthPrice: { width: '12%' },
    colWidthDiscount: { width: '12%' },
    colWidthSubt: { width: '13%' },
    colData: {
        ...BASE_COL_PROPS,
        textAlign: 'right',
        borderBottomWidth: 0,
    },
    colDataLeft: {
        ...BASE_COL_PROPS,
        textAlign: 'left',
        borderBottomWidth: 0,
    },
    totalsContainer: {
        alignSelf: 'flex-end',
        width: '40%',
        marginTop: 10,
    },
    totalRow: {
        flexDirection: 'row',
        justifyContent: 'space-between',
        paddingVertical: 2,
        fontSize: 10,
    },
    finalTotalRow: {
        flexDirection: 'row',
        justifyContent: 'space-between',
        paddingVertical: 5,
        borderTopWidth: 1,
        borderTopColor: '#000',
        fontSize: 12,
        fontWeight: 'bold',
    },
    // Footer
    footer: {
        position: 'absolute',
        bottom: 30,
        left: 30,
        right: 30,
        textAlign: 'center',
        fontSize: 8,
        color: '#666',
    },
    disclaimer: {
        fontSize: 9,
        textAlign: 'center',
        marginTop: 25,
        fontStyle: 'italic',
        color: '#444'
    }
});

// El componente ahora solo recibe la 'cotizacion'
const CotizacionComprobantePDF = ({ cotizacion }) => {
    
    if (!cotizacion) {
        return (
            <Document>
                <Page style={styles.page}>
                    <Text>Error: No se proporcionaron datos de cotización.</Text>
                </Page>
            </Document>
        );
    }

    const currentYear = new Date().getFullYear();

    // El descuento global ya viene calculado en 'montoDescuentoGlobalCalculado'
    // Usamos los otros campos solo para la etiqueta
    const descuentoGlobalLabel = cotizacion.tipoDescuentoGlobal === 'PORCENTAJE' && cotizacion.valorDescuentoGlobal > 0
        ? `Descuento Global (${cotizacion.valorDescuentoGlobal}%):`
        : 'Descuento Global:';

    
    return (
        <Document>
            <Page size="A4" style={styles.page}>
                {/* Logo y Encabezado */}
                <Image style={styles.logo} src={{ uri: LOGO_BASE64 }} />
                <Text style={styles.header}>DecoAromas - Fragancias que Cautivan</Text>
                <Text style={styles.subheader}>Cotización de Venta</Text>

                {/* Información General (Adaptada) */}
                <View style={styles.infoContainer}>
                    <View style={styles.infoBlock}>
                        <Text>Fecha de Emisión: {formatearFecha(cotizacion.fechaEmision)}</Text>
                        <Text>Vendedor: {cotizacion.usuarioNombre || 'N/A'}</Text>
                        <Text>Cliente: {cotizacion.clienteNombre || 'N/A'}</Text>
                    </View>
                </View>

                {/* Tabla de Productos */}
                <View style={styles.table}>
                    {/* Headers */}
                    <View style={styles.tableRow} fixed>
                        <Text style={{ ...styles.tableColHeader, ...styles.colWidth, borderLeftWidth: 1 }}>#</Text>
                        <Text style={{ ...styles.tableColHeader, ...styles.colWidthCode, textAlign: 'left' }}>Cód. Barras</Text>
                        <Text style={{ ...styles.tableColHeader, ...styles.colWidthProd, textAlign: 'left' }}>Producto</Text>
                        <Text style={{ ...styles.tableColHeader, ...styles.colWidthQty }}>Cant.</Text>
                        <Text style={{ ...styles.tableColHeader, ...styles.colWidthPrice }}>P. Unitario</Text>
                        <Text style={{ ...styles.tableColHeader, ...styles.colWidthDiscount }}>Desc. Unit.</Text>
                        <Text style={{ ...styles.tableColHeader, ...styles.colWidthSubt, borderRightWidth: 1 }}>Subtotal</Text>
                    </View>
                    {/* Filas (usando cotizacion.detalles) */}
                    {cotizacion.detalles.map((detalle, index) => {
                        let descuentoUnitarioDisplay = formatCurrency(0);

                        if (detalle.valorDescuentoUnitario > 0) {
                            if (detalle.tipoDescuentoUnitario === 'PORCENTAJE') {
                                descuentoUnitarioDisplay = `${detalle.valorDescuentoUnitario}%`; // Muestra "15%"
                            } else { // VALOR
                                descuentoUnitarioDisplay = formatCurrency(detalle.valorDescuentoUnitario); // Muestra "$500"
                            }
                        }

                        // Usamos el 'detalle.subtotal' que ya viene en el objeto
                        const subtotal = detalle.subtotal;

                        return (
                            <View style={styles.tableRow} key={detalle.detalleCotizacionId}>
                                <Text style={{ ...styles.colData, ...styles.colWidth, textAlign: 'center', borderLeftWidth: 1 }}>{index + 1}</Text>
                                <Text style={{ ...styles.colDataLeft, ...styles.colWidthCode }}>{detalle.codigoBarras || 'N/A'}</Text>
                                <Text style={{ ...styles.colDataLeft, ...styles.colWidthProd }}>{detalle.productoNombre}</Text>
                                <Text style={{ ...styles.colData, ...styles.colWidthQty }}>{detalle.cantidad}</Text>
                                <Text style={{ ...styles.colData, ...styles.colWidthPrice }}>{formatCurrency(detalle.precioUnitario)}</Text>
                                <Text style={{ ...styles.colData, ...styles.colWidthDiscount }}>{descuentoUnitarioDisplay}</Text>
                                <Text style={{ ...styles.colData, ...styles.colWidthSubt, fontWeight: 'bold', borderRightWidth: 1 }}>{formatCurrency(subtotal)}</Text>
                            </View>
                        );
                    })}
                </View>

                {/* Totales (usando datos del objeto cotizacion) */}
                <View style={styles.totalsContainer}>
                    <View style={styles.totalRow}>
                        <Text>Total Bruto:</Text>
                        <Text>{formatCurrency(cotizacion.totalBruto)}</Text>
                    </View>
                    {cotizacion.montoDescuentoGlobalCalculado > 0 && (
                        <View style={styles.totalRow}>
                            <Text>{descuentoGlobalLabel}</Text>
                            <Text>-{formatCurrency(cotizacion.montoDescuentoGlobalCalculado)}</Text>
                        </View>
                    )}
                    <View style={styles.finalTotalRow}>
                        <Text>TOTAL A PAGAR:</Text>
                        <Text>{formatCurrency(cotizacion.totalNeto)}</Text>
                    </View>
                </View>

                {/* Mensaje de Cotización y Footer */}
                <Text style={styles.disclaimer}>
                    Los precios y el stock están sujetos a cambios sin previo aviso.
                    Este documento no constituye un comprobante de venta.
                </Text>
                
                <Text style={styles.footer} fixed render={() => `DecoAromas - ${currentYear}`} />
            </Page>
        </Document>
    );
};

CotizacionComprobantePDF.propTypes = {
    cotizacion: PropTypes.shape({
        cotizacionId: PropTypes.number,
        fechaEmision: PropTypes.string,
        tipoCliente: PropTypes.string,
        totalBruto: PropTypes.number,
        valorDescuentoGlobal: PropTypes.number,
        tipoDescuentoGlobal: PropTypes.string,
        montoDescuentoGlobalCalculado: PropTypes.number,
        totalNeto: PropTypes.number,
        costoGeneral: PropTypes.number,
        estado: PropTypes.string,
        usuarioNombre: PropTypes.string,
        clienteNombre: PropTypes.string,
        detalles: PropTypes.arrayOf(PropTypes.shape({
            detalleCotizacionId: PropTypes.number,
            productoId: PropTypes.number,
            codigoBarras: PropTypes.string,
            productoNombre: PropTypes.string,
            cantidad: PropTypes.number,
            precioUnitario: PropTypes.number,
            valorDescuentoUnitario: PropTypes.number,
            tipoDescuentoUnitario: PropTypes.string,
            subtotal: PropTypes.number,
        })),
    }).isRequired,
};

export default CotizacionComprobantePDF;